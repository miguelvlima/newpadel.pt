...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    body{ margin:0; background:var(--bg); color:var(--fg); font-family:'Bebas Neue', system-ui,-apple-system,'Segoe UI',Roboto,Ubuntu,Cantarell,'Noto Sans',sans-serif; letter-spacing:.02em; display:flex; flex-direction:column; text-transform:uppercase; }
    ...
    main{ flex:1 1 auto; min-height:0; padding:var(--grid-pad); display:grid; gap:var(--grid-gap); grid-template-columns:repeat(2,1fr); grid-template-rows:repeat(2,1fr); place-items:stretch; overflow:hidden; }
    .tile{ height:100%; border-radius:14px; border:1px solid rgba(255,255,255,.10); background:linear-gradient(135deg,var(--tile),var(--tile-grad)); padding:var(--tile-pad); display:flex; flex-direction:column; min-height:0; }
    ...
    .badge{ display:inline-flex; align-items:center; justify-content:center; font-size: var(--fs-badge); letter-spacing:.08em; padding: var(--badge-pad-y) var(--badge-pad-x); border-radius: var(--badge-radius); border:1px solid rgba(255,255,255,.18); background:rgba(255,255,255,.06); line-height:1; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width: 100%; }
    ...
    .scoretable td.set > .cell{ display:flex; align-items:center; justify-content:center; padding:var(--pad-cell-y) var(--pad-cell-x); min-height:2.6em; background:var(--set-bg); border:1px solid var(--set-br); border-radius:12px; font-weight:900; font-size:var(--fs-set); line-height:1; color:#fff; font-variant-numeric: tabular-nums; }
    .scoretable td.now > .cell-now{ display:flex; align-items:center; justify-content:center; padding:var(--pad-cell-y) var(--pad-cell-x); min-height:2.6em; color:var(--now-fg); background:linear-gradient(180deg, rgba(0,255,163,.18), rgba(0,180,120,.14)); border:1px solid rgba(0,255,163,.45); border-radius:12px; box-shadow: 0 0 0 2px rgba(0,255,163,.20) inset, 0 0 24px rgba(0,255,163,.15); font-weight:900; font-size:var(--fs-set); line-height:1; font-variant-numeric: tabular-nums; }
    .placeholder{display:grid;place-items:center;color:#9b9b9b;font-size:18px}
    .hide-cursor *{cursor:none !important}
    @media (max-width: 900px){ header,footer{padding:10px 12px} main{gap:12px; padding:12px} .tile{padding:10px} .scoretable th, .scoretable td{ padding:6px 8px; } }
  </style>
</head>
<body>
  <header>
